...
- **Node version:** >=22
- **Dev server:** `pnpm dev`
- **Build:** `pnpm build` (runs `astro build` and then `pagefind --site dist`)
- **Deploy:** `pnpm deploy` (Wrangler/Cloudflare)
- **Lint:** `pnpm lint`
- **Format:** `pnpm format`
- **Type check:** `pnpm typecheck` (runs `astro check`)

There is no test suite configured.
Husky + lint-staged run formatting/linting checks on commit.
...
**Components:** Astro components (`.astro`) handle page-level UI (Footer, PostCard, ShareLinks, TableOfContents). React components (`.tsx`) are used for interactive widgets requiring client-side state (e.g., ThemeToggle, SearchDialog, MobileNav with `client:load`).
...
- **Class merging:** Use the `cn()` utility from `@/utils/cn` to merge Tailwind classes. Prettier is configured to sort Tailwind classes in `cn()` and `cva()` calls.
...
- **shadcn aliases:** Components are generated via `components.json` aliases (`@/components`, `@/components/ui`, `@/utils`).
